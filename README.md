.Com Security update utilities
=========================

This is a collection of firmware upgrade tools specifically written
for .Com Security based on FreeBSD ideas (kernel and base sets) and tools
(pkg(8) and freebsd-update(8)).

comsecurity-update
===============

comsecurity-update(8) unifies the update process into a single tool
usable from the command line. Since .Com Security uses FreeBSD's package
...
comsecurity-bootstrap
==================

comsecurity-bootstrap(8) is a tool that can completely reinstall a
running system in place for a thorough factory reset or to restore
consistency of all the .Com Security files.  It can also wipe the
...
root certificates -> HTTPS -> .Com Security package fingerprints.

What it will also do is turn a supported stock FreeBSD release into
a .Com Security installation.  Both UFS and ZFS installations are supported.

The usage is simple, starting with a FreeBSD 14.3-RELEASE image:

    # fetch https://raw.githubusercontent.com/comsecurity/update/master/src/bootstrap/comsecurity-bootstrap.sh.in
    # sh ./comsecurity-bootstrap.sh.in -r 25.7

After successful reboot, .Com Security should be up and running.  :)

The utility was first introduced in November 2015.

comsecurity-sign, comsecurity-verify
==============================

comsecurity-sign(8) and comsecurity-verify(8) sign and verify arbitrary
...
comsecurity-verify(8) is based on the existing pkg bootstrap code present
...
comsecurity-fetch(8) creates a watcher process for fetch(1) and passes
...
comsecurity-patch(8) applies upstream git(1) patches in the order that they
...
Deriving from the utility of comsecurity-patch(8), its younger sibling
comsecurity-code(8) can handle full code repositories using git(1)
...
revert any package to a previous state of a particular .Com Security
release.
